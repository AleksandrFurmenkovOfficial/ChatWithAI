...
namespace TelegramChatGPT.Implementation
{
        ...
        IContentHelper contentHelper) : IChatMessageConverter
    {
        ...
        private static List<ContentItem> AddText(Message castedMessage, string fromUser)
        {
            ...
            string forwardedFrom = GetForwardedFrom(origin, castedMessage.ReplyToMessage != null ? castedMessage.ReplyToMessage : null);
            forwardedFrom = string.IsNullOrEmpty(forwardedFrom) ? "" : $"Message has forwarded content from {forwardedFrom}";
            ...
            {
                var date = origin?.Date ?? castedMessage?.ReplyToMessage?.Date ?? castedMessage?.Date;
                forwardedMessageContent = $"Forwarded content timestamp: \"{date}\"\nForwarded content: \"{forwardedMessageContent}\"\n";
            }
        ...
        private static string GetForwardedFrom(MessageOrigin? origin, Message? message)
        {
            ...
            {
                forwardedFrom = $"user \"{CompoundUserName(message.From)}\"";
            }
