...
namespace ChatWithAI.Core
{
    ...
    public sealed class ChatBatchExecutor : IDisposable
    {
            ...
            CancellationToken externalCt)
        {
            ...
            // 100 threads here would be rare, but still possible.
            var batch = ClassifyEvents(events);

            // 100 batches here
            batches.Enqueue(batch); // store batch order
            // 100 batches here properly ordered and palced in queue

            // 100 threads call cancellation of previous task, first thread will wait for CancelAsync, other 99 threads will find null and skip
            await CancelCurrentTask().ConfigureAwait(false);

            // 99 threads here call RunActualTask
