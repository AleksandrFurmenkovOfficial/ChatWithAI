...
{
    public sealed class SetBaseMode(IChatModeLoader modeLoader, IMessenger messenger) : IChatCommand
    {
        public static string StaticName => "base";
        ...
        public async Task Execute(IChat chat, ChatMessageModel message, CancellationToken cancellationToken = default)
        {
            ...
            await messenger.SendTextMessage(chat.Id, new MessengerMessageDTO { TextContent = Strings.BaseModeNow }).ConfigureAwait(false);
        }
    }
}