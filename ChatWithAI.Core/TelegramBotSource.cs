    ...
    public sealed class TelegramBotSource(string telegramBotKey, IHttpClientFactory httpClientFactory) : IMessengerBotSource, IDisposable
    {
        ...
        public async Task NewBotAsync()
        {
            ...
            try
            {
                ...
                var needNew =
                    _botTask is null ||                        // ещё не создавали
                    now - _lastResetTime >= _resetInterval ||  // прошло >= 25 сек
                    _botTask.IsFaulted ||                      // прошлое создание упало
                    _botTask.IsCanceled;                       // или отменилось
