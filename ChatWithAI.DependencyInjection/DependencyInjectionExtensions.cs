...
namespace ChatWithAI.DependencyInjection
{
    public static class DependencyInjectionExtensions
    {
        ...
        {
            // Регистрируем именованный HttpClient для Telegram
            // Для 10 пользователей × 15 соединений = до 150 соединений
            ...
            // Регистрируем именованный HttpClient для Google Gemini API
            services.AddHttpClient("google_gemini_client")
                .ConfigurePrimaryHttpMessageHandler(() => new SocketsHttpHandler
                {
                    ...
                    AutomaticDecompression = DecompressionMethods.All,
                    ConnectTimeout = TimeSpan.FromSeconds(30),
                    // Для streaming SSE нужен большой таймаут на чтение
                    ResponseDrainTimeout = TimeSpan.FromMinutes(5)
                });

            // Регистрируем стандартный HttpClient для остальных сервисов
